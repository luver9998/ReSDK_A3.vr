...
class(GM_Ogryzki) extends(GMBase) //GMBase - базовый режим, от которого унаследованы все возможные режимы.
	
    // -----------------------------------------------------------------//
    // ------------------ Базовые настройки режима ---------------------//
    // -----------------------------------------------------------------//

    // Текстовые данные режима GM_Ogryzki
	var(name,"Имя режима");
	var(desc,"Описание");
	var(descExtended,"Описание в конце игры.");

	// Длительность игры в режиме GM_Ogryzki.
	var(duration,t_asMin(30)); // 30 минут
    
    // Имя карты, которая будет создана в режиме GM_Ogryzki
	getterconst_func(getMapName,"Ogryzki");
	...
	// Список ролей (имён классов) для режима GM_Ogryzki, доступных до старта раунда.
	func(getLobbyRoles)
	{
		[
            ...
            /*
                Пример:
                    "GM_Ogryzki_BasicRole1",
                    "GM_Ogryzki_BasicRole2",
                    "GM_Ogryzki_BasicRole3"
            */
		]
	};
	...
	func(conditionToStart)
	{
        ...
        /*
            Как правило здесь описываются условия типа: 
                если на "GM_Ogryzki_BasicRole1" встало 2 игрока и на "GM_Ogryzki_BasicRole2" встал 1 игрок,
            ...
            callSelfParams(getCandidatesCount,"GM_Ogryzki_BasicRole1") >= 2 
            && callSelfParams(getCandidatesCount,"GM_Ogryzki_BasicRole2") >= 1
	...
	func(onFailReasonToStart)
	{
		...
		"Не установлено свойство onFailReasonToStart() для режима GM_Ogryzki"
	};
	...
	// Вызывается когда режим GM_Ogryzki выбран (до старта раунда с загруженной картой)
	func(preSetup)
	{
		objParams();
	...
	// Событие при завершении раунда в режиме GM_Ogryzki
	func(onFinish)
	{
		objParams();
